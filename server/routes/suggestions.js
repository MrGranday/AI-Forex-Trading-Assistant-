...
//     // If Gemini API key is provided, use real AI
//     if (process.env.GEMINI_API_KEY && process.env.GEMINI_API_KEY !== 'AIzaSyAwZ3hHJjT4nOE6x7YE9Cs2lLlNU_2tf7c') {
//       try {
//         finalSuggestion = await generateEnhancedAISuggestion()
//       } catch (error) {
//         console.error('Gemini API error, falling back to mock data:', error)
//         finalSuggestion = generateMockSuggestion() // Fallback
//       }
//     } else {
...
//   const close = await getRealTimeData();
//   if (!close || close.length < 30) {
//     throw new Error('Not enough price data to generate a suggestion.');
//   }
//   const currentPrice = close[close.length - 1]
//   const rsi = RSI.calculate({ values: close, period: 14 }).slice(-1)[0]
//   const macd = MACD.calculate({ values: close, fastPeriod: 12, slowPeriod: 26, signalPeriod: 9, SimpleMAOscillator: false, SimpleMASignal: false }).slice(-1)[0]
//   const bb = BollingerBands.calculate({ period: 20, values: close, stdDev: 2 }).slice(-1)[0]
...
//     **Output Format:**
//     Return your response as a single, clean JSON object. Do not include any markdown formatting.
//     {
//       "decision": "BUY",
//       "confidence": 85,
//       "entryPrice": 1234.56,
